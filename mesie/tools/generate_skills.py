...
def _skill_body(skill_name: str, tools: List[NativeTool]) -> str:
    ...
    lines.extend([
        ...
        "2. Windows: `. .\\scripts\\MESIE.ps1` then `Invoke-MESIETool <tool-id>`",
        "3. Or: `python -m mesie.tools.cli run <tool-id>`",
        "4. Full terminal: `python -m mesie.tools.cli shell` or `Enter-MESIEShell`",
...
def _hub_skill(by_skill: Dict[str, List[NativeTool]]) -> Path:
    ...
    lines = [
        ...
        ". .\\scripts\\MESIE.ps1",
        "Get-MESIETools",
        "Invoke-MESIETool <tool-id>",
        "# or: python -m mesie.tools.cli run <tool-id>",
        "```",
        "",
        "## Skills map",
        "",
